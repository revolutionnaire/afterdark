 ...
 * Description:		Simple Maintenance Mode.
 * Version:			2.4.4
 * Tested up to:	6.2.2
 ...
 * Author:			helderk
 * Copyright:
 *  Modifications: helderk 2020-2022, Peter Hardy-vanDoorn 2018
...
//include class
include_once( plugin_dir_path( __FILE__ ) . 'hkdev_maintenance_mode_class.php' );


if (class_exists("hkdev_maintenance_mode")) {
	$hkdev_MM = new hkdev_maintenance_mode();
}
...
if (!function_exists("hkdev_codemirror_enqueue_scripts")) {
	function hkdev_codemirror_enqueue_scripts($hook) {
		$cm_settings['codeEditor'] = wp_enqueue_code_editor(array('type' => 'text/html'));
		wp_localize_script('jquery', 'cm_settings', $cm_settings);
		//wp_enqueue_script('wp-theme-plugin-editor');
		wp_enqueue_style('wp-codemirror');
		wp_enqueue_style('hkdev_select2', plugin_dir_url(__FILE__) . '/assets/select2.min.css' ); 
		wp_enqueue_script('hkdev_select2', plugin_dir_url(__FILE__) . '/assets/select2.min.js' ); 
	}
}
