...
document.addEventListener(
	...
	function() {
		...
		document.getElementById( 'location-add-button' ).addEventListener(
			'click',
			function( event ) {
		...
		document.addEventListener(
			...
			function( event ) {
				if ( event.target.matches( '.location-delete-button' ) ) {
					...
					fetch(
						guidesREST.restURL + '/delete-location/' + editor.getCurrentPostId() + '/' + i,
						{
							method: 'DELETE',
							headers: {
		...
		wp.data.subscribe(
			function() {
				...
				if ( editor.isSavingPost() && ! editor.isAutosavingPost() ) {
					...
					// Send a POST request to update the post meta.
					fetch(
						guidesREST.restURL + '/save-locations/' + editor.getCurrentPostId(),
						{
							method: 'POST',
							headers: {
								'Content-Type': 'application/json',
